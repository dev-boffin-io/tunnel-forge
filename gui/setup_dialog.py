...
class SetupDialog(QDialog):
    ...
    def _run_setup(self) -> None:
        ...
        tunnel_id = extract_tunnel_id(out) or name

        # Step 3: route DNS
        ok, _ = self._step(
            [binary, "tunnel", "route", "dns", name, domain],
            f"Route DNS {domain} → {name}",
        )
        if not ok:
            self.run_btn.setEnabled(True)
            return
        ...
        try:
            cfg_path, cfg_str = ConfigManager.generate(tunnel_id, name, domain, port)
